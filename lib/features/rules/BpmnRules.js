...
function canNotAddIncoming(element, maxIncoming) {
    return element.incoming.length >= maxIncoming;
}

function canNotAddOutgoing(element, maxOutgoing) {
    return element.outgoing.length >= maxOutgoing;
}
...
function canConnect(source, target, connection) {
  ...
  if (isSame(source, target)) {
    ...
    if(is(target, 'bpmn:StartEvent') || is(source, 'bpmn:EndEvent')) {
        return false;
    }
    if(is(source, 'bpmn:Task') && canNotAddOutgoing(source, 1)) {
        return false;
    }
    if(is(target, 'bpmn:Task') && canNotAddIncoming(target, 1)) {
        return false;
    }
